...
class LogSection:
        ...
        def start_section(self):
            print(LogSection.section0(self.msg, self.step, True,
                  depth=self.depth, title=self.title))

        def end_section(self, success=True):
            print(LogSection.section0(self.msg, self.step, False,
                  depth=self.depth, title=self.title, success=success))
            ...
            if msg != '':
                ...
                for line in msg.split('\n'):
                    if first_line and title is not None:
                        ...
                        fmtd_line = SECTION_COLOR(pad_with_spaces(f": {line}",
                            cols=fitted_cols))
...
class ItemizedLogger:
    ...
    def new_task(self, msg, title=None, subtask=False):
        ...
        self.step_stack[-1] += 1
        self.task_stack.append(LogSection(msg, self.step_stack,
            title=title))
